...
router.patch("/:id", async (req, res) => {
  ...
  // PGRST116 = no row matched; treat a PATCH on a missing template as 404
  // rather than a generic 500.
  if (error) {
    const status = error.code === "PGRST116" ? 404 : 500;
    return res.status(status).json({
      error: status === 404 ? "Template not found." : error.message,
    });
  }
  res.json(data);
});
...
router.delete("/:id", async (req, res) => {
  ...
  // Don't report success for an id that doesn't exist — an unchecked .single()
  // here previously let a DELETE on a missing template return 200 {success:true}.

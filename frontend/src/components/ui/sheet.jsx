...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-surface border-line shadow-xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
...
const SheetContent = React.forwardRef(({ side = "left", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 text-ink-muted transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-teal focus:ring-offset-2">
        <X className="h-4 w-4" />
